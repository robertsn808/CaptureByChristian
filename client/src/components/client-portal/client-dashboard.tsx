...
export function ClientDashboard({
  ...
  const { data: clientBookings } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/client-portal/bookings?clientId=${clientData.id}`).then((r) =>
        r.json(),
      ),
  });
  ...
  const { data: clientGalleries } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/client-portal/galleries?clientId=${clientData.id}`).then(
        (r) => r.json(),
      ),
  });
  ...
  const { data: clientContracts } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/client-portal/contracts?clientId=${clientData.id}`).then(
        (r) => r.json(),
      ),
  });
  ...
  const { data: clientInvoices } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/client-portal/invoices?clientId=${clientData.id}`).then((r) =>
        r.json(),
      ),
  });
  ...
  const { data: clientMessages = [] } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/client-portal/messages?clientId=${clientData.id}`).then((r) =>
        r.json(),
      ),
  });
                    ...
                    <div>
                      ...
                      <p className="text-3xl font-bold">
                        {clientGalleries?.filter(
                          (g: any) => g.status === "ready_for_download",
                        ).length || 0}
                      </p>
                    </div>
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            {format(new Date(booking.date), "MMMM dd, yyyy")} at{" "}
                            {booking.time}
                          </p>
                          <div className="mt-2">
                            ...
                            <div className="w-full bg-muted rounded-full h-2">
                              <div
                                ...
                                style={{
                                  width: `${getProgressValue(booking.status)}%`,
                                }}
                              />
                            </div>
                          </div>
                    ...
                    {clientGalleries.map((gallery: any) => (
                      <Card key={gallery.id} className="overflow-hidden">
                        <div className="aspect-video bg-muted relative">
                          <img
                            src={
                              gallery.coverImage || "/api/placeholder/400/300"
                            }
                            alt={gallery.name}
                          ...
                          <div className="flex items-center justify-between">
                            <Badge className={getStatusColor(gallery.status)}>
                              {gallery.status === "proofing"
                                ? "Select Favorites"
                                : gallery.status === "ready_for_download"
                                  ? "Ready"
                                  : gallery.status}
                            </Badge>
                            <Button
                          ...
                          <div className="flex space-x-2">
                            {contract.isFullySigned ? (
                              <Button
                                ...
                                variant="outline"
                                onClick={() =>
                                  window.open(contract.downloadUrl)
                                }
                              >
                              ...
                              <Button
                                ...
                                onClick={() => window.open(contract.signUrl)}
                              >
                    ...
                    {clientMessages.length ? (
                      <div className="space-y-3">
                        {clientMessages.map((message: any) => (
                          <div
                            key={message.id}
                            className={`flex ${message.senderId === clientData.id ? "justify-end" : "justify-start"}`}
                          >
                            <div
