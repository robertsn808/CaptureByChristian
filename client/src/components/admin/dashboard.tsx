...
export function AdminDashboard() {
  ...
  const stats = [
    ...
    },
    {
      ...
      change:
        totalBookings > 0
          ? `${Math.round((pendingBookings / totalBookings) * 100)}% pending`
          : "No data",
    ...
    },
    {
      ...
      change:
        totalBookings > 0
          ? `${Math.round((confirmedBookings / totalBookings) * 100)}% active`
          : "No data",
      changeType: "positive" as const,
    },
  ];
        ...
        <Card className="border-0 shadow-lg overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-amber-50 to-orange-50 border-b">
            <CardTitle className="flex items-center text-lg">
              ...
              <Badge
                variant={
                  pendingActions.length > 0 ? "destructive" : "secondary"
                }
                className="ml-auto"
              >
          ...
          <CardContent className="p-0">
            <div className="space-y-1">
              {pendingActions.length > 0 ? (
                pendingActions.slice(0, 4).map((booking: any) => (
                  <div
                    key={booking.id}
                    className="flex items-center justify-between p-4 hover:bg-amber-50 transition-colors border-b last:border-b-0"
                  >
                    <div className="flex items-center space-x-3">
            ...
            {pendingActions.length > 0 && (
              <div className="p-4 bg-gray-50 border-t">
                <Button
                  variant="outline"
                  className="w-full hover:bg-amber-50 border-amber-200 text-amber-700"
                >
