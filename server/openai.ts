...
export async function generateBookingResponse(
  ...
  try {
    ...
    const aiMessage = await callReplitAI(
      lastMessage,
      conversationHistory,
      bookingData,
    );
    ...
    if (aiMessage.toLowerCase().includes("wedding")) {
      ...
      aiMessage.toLowerCase().includes("aerial") ||
      aiMessage.toLowerCase().includes("drone")
    ) {
    ...
    return {
      message:
        "I'm having trouble processing your request right now. Please try again or contact us directly.",
      bookingData: {},
    };
  }
}
...
Please provide a helpful, personalized response as the AI booking assistant for CapturedCCollective media team. Include specific recommendations, pricing information when relevant, and always end with a call to action.`;
    ...
    if (!response.ok) {
      console.log(
        "Replit AI not available, falling back to rule-based responses",
      );
    ...
    return (
      ...
      generateIntelligentResponse(userMessage, conversationHistory, bookingData)
    );
  } catch (error) {
...
function generateIntelligentResponse(
  ...
  const hasDiscussedPricing =
    previousMessages.includes("price") || previousMessages.includes("cost");
  const hasDiscussedLocation =
    previousMessages.includes("location") || previousMessages.includes("where");

  // Use existing booking data if available
  const clientName = bookingData?.clientName
    ? `, ${bookingData.clientName}`
    : "";
  const preferredDate = bookingData?.date
    ? ` for ${new Date(bookingData.date).toLocaleDateString()}`
    : "";

  // Wedding photography responses
  if (
    lastMessage.includes("wedding") ||
    lastMessage.includes("marry") ||
    lastMessage.includes("bride") ||
    lastMessage.includes("groom")
  ) {
    if (
      ...
      return (
        response +
        `Would you like to check availability${preferredDate} or discuss the booking process?`
      );
    }
    if (lastMessage.includes("location") || lastMessage.includes("where")) {
      ...
      return (
        response +
        "Each location offers unique opportunities for both ground and aerial photography. Do you have a specific venue in mind, or would you like location recommendations?"
      );
    }
    if (
      lastMessage.includes("available") ||
      lastMessage.includes("date") ||
  ...
  if (
    lastMessage.includes("portrait") ||
    lastMessage.includes("family") ||
    lastMessage.includes("couple") ||
    lastMessage.includes("engagement") ||
    lastMessage.includes("maternity")
  ) {
  ...
  if (
    lastMessage.includes("drone") ||
    lastMessage.includes("aerial") ||
    lastMessage.includes("sky") ||
    lastMessage.includes("bird") ||
    lastMessage.includes("view")
  ) {
  ...
  if (
    lastMessage.includes("christian") ||
    lastMessage.includes("photographer") ||
  ...
  if (
    lastMessage.includes("special") ||
    lastMessage.includes("unique") ||
    lastMessage.includes("different") ||
...
function analyzeImageIntelligently(imageUrl: string): string {
  ...
  if (
    urlLowercase.includes("beach") ||
    urlLowercase.includes("ocean") ||
    urlLowercase.includes("sunset")
  ) {
    setting = "beachfront";
    lighting = "golden hour lighting";
  } else if (
    urlLowercase.includes("mountain") ||
    urlLowercase.includes("hike") ||
    urlLowercase.includes("landscape")
  ) {
    setting = "mountain/landscape";
    style = "environmental portrait";
  } else if (
    urlLowercase.includes("wedding") ||
    urlLowercase.includes("bride")
  ) {
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        analysis =
          data.choices?.[0]?.message?.content ||
          analyzeImageIntelligently(imageUrl);
      } else {
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        blogContent =
          data.choices?.[0]?.message?.content ||
          generateBlogContent(eventData, images);
      } else {
    ...
    const title =
      titleMatch?.[1]?.trim() ||
      `${eventData.type} Photography in ${eventData.location}`;
    ...
    const content =
      contentMatch?.[1]?.trim() ||
      `Amazing ${eventData.type.toLowerCase()} session captured in the beautiful ${eventData.location}. Christian Picaso Photography continues to deliver stunning results across Hawaii's most breathtaking locations.`;

    const excerptMatch = blogContent.match(/excerpt:?\s*(.+?)(?:\n|$)/i);
    const excerpt =
      excerptMatch?.[1]?.trim() || content.substring(0, 147) + "...";
    ...
    const tags = tagsMatch?.[1]
      ...
      eventData.location.toLowerCase().replace(/\s+/g, ""),
    ];

    const socialMatch = blogContent.match(
      /social.*?caption:?\s*(.+?)(?:\n|$)/i,
    );
    const socialCaption =
      socialMatch?.[1]?.trim() ||
